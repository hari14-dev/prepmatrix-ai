/**
 * AptitudeHubPage — 3-column category cards, skeleton loading.
 */
import { useEffect, useState } from 'react';
import { Link } from 'react-router-dom';
import { Calculator, Sigma, Brain, BookOpen, ChevronRight, CheckCircle2 } from 'lucide-react';
import { apiRequest } from '../../lib/api.js';
import { useAuth } from '../../context/AuthContext.jsx';

const CATS = [
  { key:'Quant',   label:'Quantitative',     sub:'Arithmetic, Algebra, Geometry',   Icon:Sigma,  accent:'var(--cyan)',   dim:'var(--cyan-dim)'   },
  { key:'Logical', label:'Logical Reasoning', sub:'Puzzles, Patterns, Sequences',    Icon:Brain,  accent:'var(--indigo-light)', dim:'var(--indigo-dim)' },
  { key:'Verbal',  label:'Verbal Ability',    sub:'Reading, Grammar, Vocabulary',    Icon:BookOpen, accent:'var(--violet)', dim:'var(--violet-dim)' },
];

export function AptitudeHubPage() {
  const { token } = useAuth();
  const [hub, setHub] = useState({ Quant:[], Logical:[], Verbal:[] });
  ...
  useEffect(() => {
    if (!token) return;
    setIsLoading(true);
    apiRequest('/api/aptitude/hub', { token })
  ...
  return (
    <div className="animate-in">
        ...
        {!isLoading && allTopics.length>0 && (
          <div style={{ display:'flex', alignItems:'center', gap:'0.65rem', flexWrap:'wrap' }}>
            <div className="solved-chip">
              ...
              <span>{doneTopics} / {allTopics.length} completed</span>
            </div>
      ...
      <div className="aptitude-cats-grid">
        {CATS.map(cat => {
                ...
                <article key={cat.key} className="subject-card">
                  ...
                  <div className="subject-card-topics">
                    {topics.length===0
                      ...
                      : topics.map(t => (
                          <Link key={t.slug} to={`/dashboard/aptitude/topic/${t.slug}`} className="topic-row">
